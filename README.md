...
![PDFExtractor icon](assets/pdf-html.png)

PDFExtractor is a local-first Linux desktop application for converting PDFs into
structured JSON, Markdown, HTML, or a searchable SQLite knowledge base.

**Two conversion paths are selected automatically for each PDF:**

| PDF | Path | Typical speed |
|---|---|---|
| Has an embedded text layer (most research papers) | Direct `pdftext` extraction with per-page verification | Seconds |
| Scanned/image-only | [Marker](https://github.com/datalab-to/marker) + Surya OCR | Minutes to hours |

The fast path does not import Marker or PyTorch, load OCR models, or need roughly
8 GB of RAM. Every page is compared with the raw PDF text layer. Missing characters
are recovered and reported; unreadable pages are isolated instead of aborting the PDF.
...
- The first launch creates a per-user virtual environment but downloads no OCR
  models. Marker/Surya models are fetched only when a scanned PDF needs them.
- Optional semantic search downloads BAAI/bge-m3 only when embeddings are enabled.
...
Python runtime dependencies, then opens immediately without downloading OCR models.
Those models are fetched on demand only for scanned PDFs.
...
Backend μετατροπής: άμεση ανάγνωση text layer με `pdftext` και, μόνο όταν
χρειάζεται, [Marker](https://pypi.org/project/marker-pdf/) με Surya OCR.

## Μνήμη και OCR

Η μνήμη μετράει μόνο για σκαναρισμένα PDF. Το Marker χρειάζεται περίπου **8 GB
διαθέσιμη RAM**· η γρήγορη διαδρομή text-layer είναι ελαφριά και δεν φορτώνει τα
μοντέλα. Η εφαρμογή δείχνει ⚡ ή 🐌 για κάθε αρχείο πριν ξεκινήσει.
...
#   επιλογές: --format md|html|json  --out DIR  --force-ocr
#   προαιρετική εκ των προτέρων λήψη OCR μοντέλων:
...
```bash
./run.sh            # [1] PDF→HTML/JSON GUI (άμεσο άνοιγμα· OCR on demand)
...
./install.sh        # launcher «PDFExtractor» στο μενού εφαρμογών
```

**Τρία GUI, σκόπιμα χωριστά:** [1] μόνο μετατροπή,
...
1. **Convert** (text layer ή, μόνο αν χρειάζεται, Marker/OCR) — γράφει και JSON reports.
...
Αποτέλεσμα: `output/<όνομα>/` με `<όνομα>.db` + `json/` + `README.md` (manifest). Το πρωί
...
format JSON/Markdown/HTML/**Βάση + JSON — συγγραφή & έλεγχος**· κουμπί
«Μετατροπή» + spinner + progress. Η συνδυασμένη επιλογή γράφει ένα queryable
`papers.db` και φάκελο `json/` στον ίδιο προορισμό: τα πλήρη JSON χρησιμοποιούνται
για συγγραφή με διατήρηση context, ενώ η βάση χρησιμοποιείται για αναζήτηση,
διασταύρωση και έλεγχο ακρίβειας. Η βάση περιέχει `papers`, `sections`, `blocks`
και full-text search.
...
Το εγκατεστημένο πακέτο στήνει **per-user venv**. Τα OCR μοντέλα κατεβαίνουν on demand.

## Αρχεία

| Αρχείο | Ρόλος |
|---|---|
| `fasttext_extract.py` | Γρήγορη εξαγωγή text layer, headings, verification και recovery ανά σελίδα |
| `convert.py` | Αυτόματη επιλογή fast path ή Marker/OCR → JSON/MD/HTML + report |
...
| `download_models.py` / `firstrun.py` | Προαιρετική εκ των προτέρων λήψη OCR μοντέλων |
