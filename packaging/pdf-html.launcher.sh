...
# τρέχει τον first-run wizard. Οι πηγές ζουν read-only στο /opt/pdf-html· το
...
set -euo pipefail
SRC="/opt/pdf-html"
APP="${PDFHTML_HOME:-$HOME/.local/share/pdf-html}"
mkdir -p "$APP"
...
if [ ! -x ".venv/bin/python" ] || ! .venv/bin/python -c 'import marker' 2>/dev/null; then
  ...
  "$UV" pip install --python .venv/bin/python marker-pdf tkinterdnd2 sentence-transformers sqlite-vec pip
fi
